...
def main(
    nuke_executable: click.Path, test_dir: click.Path, config: click.Path, interactive: bool, pytest_arg: list
) -> NoReturn:
    ...
    """
    if not config and not (nuke_executable and test_dir):
        msg = (
            "No config specified or not a nuke executable with test dir. "
            "You need to specify at least one of those two. Use --help for "
